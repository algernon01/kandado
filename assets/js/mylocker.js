(() => {
  ...
  function renderTick(){
    ...
    // --- CHANGED: show days when >= 24 hours ---
...
/* --- CHANGED: fetch the QR as a Blob so the page never navigates --- */
if (saveBtn && qrImage){
  saveBtn.addEventListener('click', async (e) => {
    e.preventDefault();
    const dlUrl = qrImage.src + (qrImage.src.includes('?') ? '&' : '?') + 'dl=1';
    try{
      const u = new URL(dlUrl, window.location.href);
      const creds = (u.origin === window.location.origin) ? 'same-origin' : 'include';
      const resp = await fetch(u.toString(), { credentials: creds, cache: 'no-store' });
